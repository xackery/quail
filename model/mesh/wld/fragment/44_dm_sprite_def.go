...
type SpriteVertexTex struct {
	X int16
	Y int16
}

func LoadDmSpriteDef(r io.ReadSeeker) (archive.WldFragmenter, error) {
	e := &DmSpriteDef{}
	err := parseDmSpriteDef(r, e)
	if err != nil {
		return nil, fmt.Errorf("parse DmSpriteDef: %w", err)
	}
	return e, nil
}

func parseDmSpriteDef(r io.ReadSeeker, v *DmSpriteDef) error {
	if v == nil {
		return fmt.Errorf("DmSpriteDef is nil")
	}
	var err error
	...
	if err != nil {
		return fmt.Errorf("read Sprite mesh: %w", err)
	}
...
func (e *DmSpriteDef) FragmentType() string {
	return "DmSpriteDef"
}

func (e *DmSpriteDef) Data() []byte {
