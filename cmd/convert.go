...
func init() {
	...
	convertCmd.PersistentFlags().Bool("strip", false, "strip out lit/vislist data that is likely to be regenerated by external programs")
}
...
func runConvertE(cmd *cobra.Command, args []string) error {
	...
	isStripped, err := cmd.Flags().GetBool("strip")
	if err != nil {
		return fmt.Errorf("get strip flag: %w", err)
	}

	if isStripped {
		fmt.Println("--strip flag was used")
	}
