...
func Test_export_import_cmp(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err != nil {
				t.Errorf("newFile: %s", err.Error())
				return
			}
			c1, err := mod.NewFile(tt.fileName, pfs, tt.fileName)
			if err != nil {
				t.Errorf("mod newfile: %s", err.Error())
				return
			}
			...
			err = c2.Encode(buf2)
			if err != nil {
				t.Errorf("c2 Encode: %s", err.Error())
				return
			}
			w1, err := os.Create("test/c1.hex")
			if err != nil {
				t.Errorf("os.Create: %s", err.Error())
				return
			}
			_, err = w1.Write(buf1.Bytes())
			if err != nil {
				t.Errorf("w1.Write: %s", err.Error())
				return
			}
			w1.Close()
			...
			if len(buf1.Bytes()) != len(buf2.Bytes()) {
				t.Errorf("buf1 and buf2 are not the same length")
				return
			}

			for offset, v := range buf1.Bytes() {
				if buf2.Bytes()[offset] != v {
					t.Errorf("buf2 does not match buf1 at offset %d", offset)
					return
				}
			}
			...
			if err != nil {
				t.Errorf("eqg1.ReadFile: %s", err.Error())
				return
			}
			eqgData2, err := os.ReadFile(fmt.Sprintf("test/%s", tt.name))
			if err != nil {
				t.Errorf("eqg2.ReadFile: %s", err.Error())
				return
			}
			if len(eqgData) != len(eqgData2) {
				t.Errorf("eqg1 and eqg2 are not the same length")
				return
			}
			for i, v := range eqgData {
				if eqgData2[i] != v {
					t.Errorf("eqg2 does not match eqg1 at offset %d", i)
					return
				}
			}
