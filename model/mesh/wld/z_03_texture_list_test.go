...
func TestWLD_textureListRead(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "gequip.s3d", wantErr: false}, // TODO: texture count is 0 on all examples
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			count, err := parseFragments(fmt.Sprintf("%s/test_data/%s", eqPath, tt.name), 3, e.textureListRead)
