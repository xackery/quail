...
func importFile(req *ObjRequest) error {
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "f ") {
			...
			req.Data.Triangles = append(req.Data.Triangles, &geo.Triangle{
				Index:        &geo.UIndex3{X: uint32(index1), Y: uint32(index2), Z: uint32(index3)},
...
func triangle(v, t, n int, objCache *objCache, obj *ObjData) (int, error) {
	...
	obj.Vertices = append(obj.Vertices, &geo.Vertex{
		Position: vert,
		Normal:   norm,
		Uv:       uv,
	})
